...
def prepare_data(data_path, labels2id, batch_size, baseline):
    sentences, entities_1, entities_2, relations = read_json_file(data_path, labels2id, baseline)
...
def read_json_file(json_file, labels2id, baseline, multi_label=False):
    ...
    with open(json_file) as data_file:
        for json_elem in data_file:
            ...
            domain = document["doc_key"].split("-")[0].strip()
            ...
            if len(document["ner"]) > 1:
                ...
                for entity_pair in entity_pairs:
                    ...
                    if not baseline:
                        ...
                        #sentence_marked = f'{domain.capitalize()} [SEP] '
                        #sentence_marked = f'[{domain}] [SEP] '
                    else:
                        sentence_marked = ''
                    ...
                    if len(dataset_relations) > 0:
                        ...
                        #sentence_marked += f' [SEP]'
                        #print(sentence_marked.strip())
