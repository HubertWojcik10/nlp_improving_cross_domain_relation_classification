...
def parse_arguments():
    ...
    arg_parser.add_argument('--baseline', type=str, nargs='?', required=True, help='Whether the baseline is used or not')

    return arg_parser.parse_args()

def get_metrics(gold_path, predicted_path):
    if args.baseline.lower() == 'true':
        args.baseline = True
    else:
        args.baseline = False
    ...
    _, _, _, gold_relations = read_json_file(gold_path, label_types, args.baseline, multi_label=True)
