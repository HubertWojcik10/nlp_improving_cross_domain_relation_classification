...
def parse_arguments():
    ...
    arg_parser.add_argument('-base', '--baseline', type=str, help='Whether to use baseline model or not')
...
if __name__ == '__main__':
    ...
    # setup data
    domains = []
    print(f'is baseline: {args.baseline}')
    if args.prediction_only:
        test_data = prepare_data(args.test_path, label_types, args.batch_size, args.baseline)
        logging.info(f"Loaded {test_data} (test).")
    else:
        train_data = prepare_data(args.train_path, label_types, args.batch_size, args.baseline)
        logging.info(f"Loaded {train_data} (train).")
        dev_data = prepare_data(args.dev_path, label_types, args.batch_size, args.baseline)
