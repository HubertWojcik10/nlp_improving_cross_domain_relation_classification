...
LM='bert-base-cased'
SEEDS=( 5001 ) # 5000 - baseline, 5001 - model
BASELINE=false
...
for rs in "${!SEEDS[@]}"; do
  ...
  # if no prediction is available, run inference
    ...
    python3 main.py \
            --train_path $DATA_PATH/${DOMAIN}-train.json \
            --test_path $DATA_PATH/${TEST_DOMAIN}-test.json \
            --exp_path ${exp_dir} \
            --language_model ${LM} \
            --seed ${SEEDS[$rs]} \
            --prediction_only \
  ...
  # run evaluation
  python3 evaluate.py \
          --gold_path ${DATA_PATH}/${TEST_DOMAIN}-test.json \
          --pred_path ${exp_dir}/${TEST_DOMAIN}-test-pred.csv \
          --out_path ${exp_dir} \
